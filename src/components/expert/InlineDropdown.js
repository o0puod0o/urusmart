...
import { ActivityIndicator, View, Text, TouchableOpacity, ScrollView, TextInput } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import { useTranslation } from "react-i18next";

const InlineDropdown = ({ label, value, options, onSelect, required, searchable = false, loading = false }) => {
  ...
  return (
    <View className="px-4 py-3">
      ...
      {loading ? (
        <View className="flex-row items-center px-3 py-[13px] border border-[#e8ecf0] rounded-[10px] bg-[#f8fafb]">
          <ActivityIndicator size="small" color="#007a5a" />
          <Text className="ml-2 text-[13px] text-[#888]">กำลังโหลด...</Text>
        </View>
      ) : (
        <>
          <TouchableOpacity
