...
const normalizeTraining = (item) => ({
  id: String(item.id ?? item.training_id ?? Date.now()),
  year: String(item.year ?? item.training_year ?? item.study_year ?? ""),
  title: item.title ?? item.topic ?? item.training_title ?? item.name ?? "",
});
...
const TrainingForm = ({ navigation }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const [trainingRes] = await Promise.allSettled([
          fetch(`${BASE_URL}/api/trainings`),
        ]);
        if (trainingRes.status === "fulfilled" && trainingRes.value.ok) {
          const data = await trainingRes.value.json();
          const rows = Array.isArray(data) ? data : (data.data ?? []);
          setItems(rows.map(normalizeTraining));
        }
      } catch {
        console.log("Training API not ready");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    if (!form.year || !form.title.trim()) {
      Alert.alert("แจ้งเตือน", "กรุณาเลือกปีและกรอกหัวข้อ");
      return;
    }
    ...
    try {
      ...
      const endpoint = isEditing
        ? `${BASE_URL}/api/trainings/${editingItem.id}`
      ...
      const saved = normalizeTraining(savedData.data ?? savedData ?? payload);
  ...
  const handleDelete = (entry) => {
    Alert.alert("ลบข้อมูล", "ต้องการลบรายการนี้ใช่ไหม?", [
      { text: "ยกเลิก", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const res = await fetch(`${BASE_URL}/api/trainings/${entry.id}`, {
              method: "DELETE",
            });
  ...
  return (
    <View style={styles.container}>
      <AppHeader
        title="ประวัติการฝึกอบรม/ศึกษาดูงาน"
      ...
      >
        ...
        <View style={styles.hero}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.heroTitle}>ประวัติการฝึกอบรม/ศึกษาดูงาน</Text>
          </View>
        ...
        <View style={styles.tableCard}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardHeaderText}>
              รายการประวัติการฝึกอบรม/ศึกษาดูงาน
            </Text>
          </View>
          ...
          {loading ? (
            ...
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyText}>
                ยังไม่มีข้อมูลประวัติการฝึกอบรม/ศึกษาดูงาน
              </Text>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={styles.table}>
                <View style={styles.tableHead}>
                  ...
                  <Text style={[styles.th, styles.col_title]}>หัวข้อ</Text>
        ...
        <View style={styles.formCard}>
          <View style={styles.formHeader}>
            <View style={styles.formHeaderLeft}>
              ...
              <Text style={styles.formTitle}>
                {editingItem
                  ? "แก้ไขข้อมูลการฝึกอบรม/ศึกษาดูงาน"
                  : "เพิ่มข้อมูลการฝึกอบรม/ศึกษาดูงาน"}
              </Text>
            </View>
          ...
          <InlineDropdown
            label="ปี:"
            value={form.year}
            options={GENERATED_YEAR_OPTIONS}
          ...
          <View style={styles.fieldWrap}>
            <Text style={styles.fieldLabel}>
              หัวข้อ:<Text style={styles.required}> *</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={form.title}
              onChangeText={(v) => setField("title", v)}
              placeholder="ระบุหัวข้อการฝึกอบรม/ศึกษาดูงาน"
